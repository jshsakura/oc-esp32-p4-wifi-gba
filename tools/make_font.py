...
def main():
    ...
    ap.add_argument("--c-array", metavar="SYMBOL",
                    help="also emit <out>.c holding the same bytes as a C array, to compile in")
    args = ap.parse_args()
    ...
    blob = bytes(header + glyphs)
    with open(args.out, "wb") as f:
        f.write(blob)

    if args.c_array:
        # Same bytes, just reachable from the firmware image. rg_font_load_from_memory parses
        # them at boot, so there is one parser rather than a second C-struct emitter here.
        lines = []
        for i in range(0, len(blob), 16):
            lines.append("    " + " ".join(f"0x{b:02X}," for b in blob[i:i + 16]))
        with open(args.out + ".c", "w") as f:
            f.write(f"// Generated by tools/make_font.py -- do not edit.\n"
                    f"// Source: {args.font} at {args.height}px\n"
                    f"#include <stddef.h>\n#include <stdint.h>\n\n"
                    f"const uint8_t {args.c_array}[] = {{\n" + "\n".join(lines) +
                    f"\n}};\nconst size_t {args.c_array}_size = sizeof({args.c_array});\n")
        print(f"  also wrote {args.out}.c ({args.c_array})")
