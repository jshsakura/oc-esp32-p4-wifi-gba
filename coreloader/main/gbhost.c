...
/* ---- the symbol table -------------------------------------------------------
 *
 * Generated, not hand-written: tools/generate_symbols.py reads the module's
 * undefined symbols and emits gbhost_symbols.c plus a human-readable contract
 * (gnuboy_module.imports.txt). A core's imports ARE its contract with the host,
 * so the list is a build artifact somebody can read, not something that only
 * exists inside a .o.
 *
 * gnuboy asks for 18. Seventeen are libc and libgcc; the only framework call is
 * rg_system_log. The libgcc soft-double entries are the tell that this is
 * genuinely separate code -- gnuboy's RTC uses doubles and this RISC-V has none
 * in hardware, so the module calls back into the host's helpers.
 */
