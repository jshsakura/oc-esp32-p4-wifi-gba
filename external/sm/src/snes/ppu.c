...
#ifndef TARGET_GNW
/* types.h already defines uint as `unsigned int`, and this has to agree with it.
 * On ARM it did by accident -- uint32_t IS unsigned int there, so the two spellings
 * named one type and nothing complained. This toolchain has uint32_t as
 * `long unsigned int`, which makes them different types and the redefinition an
 * error. Same width either way; only the spelling was ever load-bearing. */
