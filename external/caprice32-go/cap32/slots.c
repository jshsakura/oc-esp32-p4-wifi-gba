...
// A second, compact save-state format for the Game & Watch port, built on
// amstradSaveState*() (declared nowhere in this tree) and on t_drive fields
// (loaded_track/loaded_side) that only exist in the TARGET_GNW half of the
// #ifdef in cap32.h. Upstream ships both functions unguarded even though
// they cannot build outside TARGET_GNW, and nothing outside this file calls
// either one -- snapshot_save_mem()/snapshot_load_mem() below are the SNA
// path every other port (including us) actually uses.
#ifdef TARGET_GNW
