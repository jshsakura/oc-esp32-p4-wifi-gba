...
// Not const upstream, which makes the linker keep a full RAM-resident copy of
// every one of these ROM images regardless of which CPC model is actually
// selected (~114KB combined, more than internal DRAM has room for once the
// rest of retro-go is linked in) -- they are only ever memcpy()'d FROM, so
// flash residency changes nothing at runtime.
const uint8_t OS_BASIC10[32768] = {
