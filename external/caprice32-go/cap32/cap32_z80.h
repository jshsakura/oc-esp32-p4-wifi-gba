...
typedef struct {
   ...
   // Upstream gates these on TARGET_GNW, but cap32_z80.c reads them in the
   // !TARGET_GNW build (cap32_z80_execute()'s breakpoint loop and trace exit
   // condition) and never the other way around -- with the #ifdef as shipped,
   // the non-TARGET_GNW build (the one every port other than the Game & Watch
   // one uses, including the actual libretro core) can't see these fields at
   // all and fails to compile. Unconditional matches how they're actually used.
