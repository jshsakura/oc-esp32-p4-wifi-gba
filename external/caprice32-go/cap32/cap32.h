...
// driveA/driveB (defined in cap32.c) are ~168KB each with the !TARGET_GNW
// layout above -- t_track[DSK_TRACKMAX][DSK_SIDEMAX] statically sized for the
// worst-case DSK geometry regardless of the disk actually loaded. Both fit
// in a desktop's BSS without a second thought, but 336KB combined does not
// fit alongside the rest of retro-go in an ESP32's internal RAM. They are
// heap-allocated once, in caprice_emulator_init(), through retro_malloc()
// (PSRAM) instead -- and referenced through these macros so every existing
// `driveA.foo` / `&driveA` callsite across this fork keeps compiling as-is.
extern t_drive *driveA_ptr;
extern t_drive *driveB_ptr;
#define driveA (*driveA_ptr)
#define driveB (*driveB_ptr)
