...
void v810_init(void) {
...
#ifdef GNW_VB_DEVICE
    // Device: ROM is XIP'd from QSPI flash — app_main_vb() sets pmemory/size/off
    // from the getromdata() pointer. No 16MB malloc (won't fit 724KB RAM_EMU).
    V810_ROM1.pmemory = NULL;
#else
    ...
    // On device the ~338KB of RAM regions must come from the RAM_EMU emulator
    // heap (ram_calloc), NOT newlib malloc — vb_dev_calloc is provided by the
    // firmware glue (main_vb.c). The host harness maps it to calloc so it tests
    // this exact allocation path.
#ifdef GNW_VB_DEVICE
    extern void *vb_dev_calloc(size_t nmemb, size_t size);
    #define VB_REGION_CALLOC(n, s) vb_dev_calloc((n), (s))
#else
    #define VB_REGION_CALLOC(n, s) calloc((n), (s))
#endif

#ifdef GNW_VB_DEVICE
    // Single player only (no link cable): allocate one player's RAM → 338KB not 676KB.
    for (int i = 0; i < 1; i++) {
#else
    for (int i = 0; i < 2; i++) {
#endif
        ...
        vb_players[i].V810_DISPLAY_RAM.pmemory = (unsigned char *)VB_REGION_CALLOC(vb_players[i].V810_DISPLAY_RAM.size, sizeof(BYTE));
        // Offset + Lowaddr = pmemory
        ...
        // Alocate space for it in memory
        vb_players[i].V810_SOUND_RAM.pmemory = (unsigned char *)VB_REGION_CALLOC(vb_players[i].V810_SOUND_RAM.size, sizeof(BYTE));
        // Offset + Lowaddr = pmemory
        ...
        // Alocate space for it in memory
        vb_players[i].V810_VB_RAM.pmemory = (unsigned char *)VB_REGION_CALLOC(vb_players[i].V810_VB_RAM.size, sizeof(BYTE));
        // Offset + Lowaddr = pmemory
        ...
        // Alocate space for it in memory
        vb_players[i].V810_GAME_RAM.pmemory = (unsigned char *)VB_REGION_CALLOC(vb_players[i].V810_GAME_RAM.size, sizeof(BYTE));
        // Offset + Lowaddr = pmemory
...
#endif  /* interpreter (device) uses serviceInt() directly; these fields are DRC-only */
