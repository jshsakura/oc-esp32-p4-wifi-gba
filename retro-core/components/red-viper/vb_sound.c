...
#ifdef GNW_VB_DEVICE
/* Route the ~15KB of wave buffers to the RAM_EMU emulator heap (bump allocator,
 * so free is a no-op) instead of newlib malloc. vb_dev_malloc is provided by the
 * firmware glue (main_vb.c); the host harness maps it to malloc. */
extern void *vb_dev_malloc(size_t size);
#undef linearAlloc
#undef linearFree
#define linearAlloc(sz) vb_dev_malloc(sz)
...
uint8_t sound_fill_buf = 0;

static uint16_t buf_pos = 0;
static int16_t *wavebufs[BUF_COUNT];
...
static void fill_buf_single_sample(int ch, int samples, int offset) {
    ...
    for (int i = 0; i < samples; i++) {
        ((uint32_t*)(wavebufs[sound_fill_buf]))[offset + i] += total;
    }
}
...
void vb_sound_update(uint32_t cycles) {
    ...
    while (remaining_samples > 0) {
        ...
        memset(wavebufs[sound_fill_buf] + buf_pos * 2, 0, sizeof(int16_t) * samples * 2);
        ...
        if (buf_pos == SAMPLE_COUNT) {
            ...
            for (int i = 0; i < SAMPLE_COUNT; i++) {
                ...
                short left = AMPLIFY(wavebufs[sound_fill_buf][i * 2]) + dc_offset;
                short right = AMPLIFY(wavebufs[sound_fill_buf][i * 2 + 1]) + dc_offset;
                #undef AMPLIFY
                ...
                if (dc_offset != 0) {
                    wavebufs[sound_fill_buf][i * 2] = left;
                    wavebufs[sound_fill_buf][i * 2 + 1] = right;
                }
            }
            // push
            if (sound_push_backend(wavebufs[sound_fill_buf])) {
                sound_fill_buf = (sound_fill_buf + 1) % BUF_COUNT;
            }
            buf_pos = 0;
        }
    }
...
void sound_write(int addr, uint16_t data) {
    ...
    if (!(addr & 0x400)) {
        ...
        vb_sound_update(vb_state->v810_state.cycles);
    }
...
void sound_refresh(void) {
    ...
    for (int i = 0; i < BUF_COUNT; i++) {
        memset(wavebufs[i], 0, SAMPLE_COUNT * 4);
    }
    sound_resume();
}

void vb_sound_init(void) {
    for (int i = 0; i < BUF_COUNT; i++) {
        wavebufs[i] = linearAlloc(sizeof(int16_t) * SAMPLE_COUNT * 2);
    }
    if (!sound_init_backend(wavebufs)) {
        for (int i = 0; i < BUF_COUNT; i++) {
            linearFree(wavebufs[i]);
        }
    }
}
...
void sound_close(void) {
    sound_close_backend();
    for (int i = 0; i < BUF_COUNT; i++) linearFree(wavebufs[i]);
}
...
void sound_resume(void) {
    sound_resume_backend();
}

void vb_sound_reset(void) {
