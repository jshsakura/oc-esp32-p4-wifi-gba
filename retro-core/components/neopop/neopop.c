...
#else
	system_message("[PC %06X] %s", ngp_pc, message);
#endif
}
...
{
	if (ngp_pc >= 0x7000 && ngp_pc <= 0x7FFF)
	{
		char* s;
		_u16 p = (_u16)(ngp_pc & 0xFFF);
		s = Z80_disassemble(&p);
		ngp_pc = p + 0x7000;
		return s;
	}
...
void emulate_debug(BOOL dis_TLCS900h, BOOL dis_Z80)
{
	...
	if (dis_Z80)
	{
		...
		{
			char* s;
			_u16 ngp_pc = Z80_getReg(Z80_REG_PC);
			_u16 store_pc = ngp_pc;

			//Disassemble
			s = Z80_disassemble(&ngp_pc);
			system_debug_message(s);
