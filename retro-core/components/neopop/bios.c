...
_u8 ngp_bios[0x10000];		//Holds ngp_bios program data
...
{
	//=== Install the reverse engineered ngp_bios
	int i;
	...
	for (i = 0; i <= 0x1A; i++)
	{
		*(_u32*)(ngp_bios + 0xFE00 + (i * 4)) = htole32(vectable[i]);
		ngp_bios[vectable[i] & 0xFFFF] = 0x1F;	//iBIOSHLE
	}

	//System Font
	memcpy(ngp_bios + 0x8DCF, font, 0x800);

	//Default Interrupt handler
	ngp_bios[0x23DF] = 0x07;  //RETI

	// ==========
		
	//Install a Quick and Dirty Bios
	ngp_bios[0xFFFE] = 0x68; // - JR 0xFFFFFE (Infinite loop!)
	ngp_bios[0xFFFF] = 0xFE;
