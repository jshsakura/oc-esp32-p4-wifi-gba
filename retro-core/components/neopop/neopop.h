...
- Moved reset() and biosInstall (now bios_install) from ngp_bios.h
...
/* Fill the ngp_bios rom area with a ngp_bios. call once at program start */
...
/*!	Fills the given buffer with sound data */
	...
	void ngp_sound_update(_u16* chip_buffer, int length_bytes);
	void dac_update(_u8* dac_buffer, int length_bytes);

/*! Initialises the sound chips using the given SampleRate */
	
	void ngp_sound_init(int SampleRate);

		//=========================================

/*! Callback for "ngp_sound_init" with the system sound frequency */
