...
// below now need _u8/_u32/ram/rom/ngp_bios/flash_write/timer_hint directly.
#include "neopop.h"
#include "bios.h"
#include "flash.h"
#include "interrupt.h"
...
static inline void* translate_address_read(_u32 address)
{
	...
	if ((address & 0xFF0000) == 0xFF0000)
		return ngp_bios + (address & 0xFFFF); // BIOS ROM
...
static inline void* translate_address_write(_u32 address)
{
	...
	else
	{
		...
		if (rom.data && address >= ROM_START && address <= ROM_END)
		{
			...
			{
	//			system_debug_message("%06X: Enable EEPROM command from %06X", ngp_pc, address);
			...
			if (address == 0x220000 || address == 0x230000)
			{
	//			system_debug_message("%06X: EEPROM status read from %06X", ngp_pc, address);
	...
	//			system_debug_message("%06X: Direct EEPROM write to %06X", ngp_pc, address & 0xFFFF00);
	//			system_debug_stop();
