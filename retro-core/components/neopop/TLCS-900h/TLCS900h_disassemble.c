...
_u8 get8_dis(void)
{
	_u8 a = bytes[bcnt++] = loadB(ngp_pc++);
	return a;
}

_u16 get16_dis(void)
{
	_u16 a = *(_u16*)(_u8*)(bytes + bcnt) = loadW(ngp_pc);
	ngp_pc += 2; bcnt += 2;
	return a;
}
...
{
	_u8 b; _u16 a;
	a = *(_u16*)(_u8*)(bytes + bcnt) = loadW(ngp_pc);
	ngp_pc += 2; bcnt += 2;
	b = bytes[bcnt++] = loadB(ngp_pc++);
	return ((_u32)b << 16) | (_u32)a;
}

_u32 get32_dis(void)
{
	_u32 a = *(_u32*)(_u8*)(bytes + bcnt) = loadL(ngp_pc);
	ngp_pc += 4; bcnt += 4;
	return a;
}
...
static void CALR()
{
	sprintf(instr, "CALR 0x%06X", (_s16)get16_dis() + ngp_pc);
}
...
static void JR()
{
	sprintf(instr, "JR %s,0x%06X", ccName[first & 0xF], (_s8)get8_dis() + ngp_pc);
}

static void JRL()
{
	sprintf(instr, "JRL %s,0x%06X", ccName[first & 0xF], (_s16)get16_dis() + ngp_pc);
}
