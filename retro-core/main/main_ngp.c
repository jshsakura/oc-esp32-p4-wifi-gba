...
// Named NGP_AUDIO_SAMPLE_RATE rather than AUDIO_SAMPLE_RATE: that name is
...
#define NGP_AUDIO_SAMPLE_RATE    22050
#define NGP_AUDIO_BUFFER_LENGTH  (NGP_AUDIO_SAMPLE_RATE / NGP_FPS_APPROX + 1)
...
// Mono scratch buffer filled by the core's ngp_sound_update().
static _u16 chip_buffer[NGP_AUDIO_BUFFER_LENGTH];
...
{
    ngp_sound_init(NGP_AUDIO_SAMPLE_RATE);
}
...
void ngp_main(void)
{
    ...
    app = rg_system_reinit(NGP_AUDIO_SAMPLE_RATE, &handlers, NULL);
    ...
    while (true)
    {
        rg_audio_sample_t mixbuffer[NGP_AUDIO_BUFFER_LENGTH];
        ...
        ngp_sound_update(chip_buffer, samples * (int)sizeof(_u16));
