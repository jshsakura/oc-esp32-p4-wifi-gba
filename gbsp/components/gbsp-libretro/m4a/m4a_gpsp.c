...
/* gpSP internals we borrow.
 *
 * On the Game & Watch these were declared by hand, because including gpSP's
 * headers pulls in a "common.h" that collides with that firmware's own. Here it
 * does not: this file is compiled inside the gpSP component, so "common.h" IS
 * gpSP's and there is no second one to collide with — and taking the header is
 * not merely allowed, it is required. This port keeps gpSP's big buffers in one
 * heap-allocated struct and reaches them through macros
 * (gba_memory.h: `#define memory_map_read gbsp_memory->memory_map_read`), so
 * there is no flat `memory_map_read` symbol to link against. Restating that
 * struct's layout here would put an offset in a file nothing checks; the header
 * is the one copy that stays right.
 *
 * What this brings, and what the hand-declared version used to spell out:
 *   gba_memory.h  memory_map_read, ws_cyc_seq, ws_cyc_nseq, reg
 *   cpu.h         REG_PC=15, REG_CPSR=16, CPU_HALT_STATE=18, CPU_ACTIVE=0
 *   main.h        update_gba, cpu_ticks, execute_cycles,
 *                 completed_frame(c) = (c) & 0x80000000,
 *                 cycles_to_run(c)   = (c) & 0x7FFF
 * Getting cycles_to_run's mask wrong (it is 0x7FFF, not a 30-bit one) hands the
 * CPU a budget of hundreds of thousands of cycles and the emulated machine
 * simply stops advancing — which is why it is now taken from the header rather
 * than copied. */
#include "common.h"
...
static unsigned char *m4a_map(void *ctx, uint32_t addr, uint32_t *span)
{
    ...
    if (!page)
    {
        /* A cart page that is not resident. On the Game & Watch this could not
         * happen — the cart was execute-in-place, so every ROM address was always
         * mapped and this branch was belt and braces. Here the ROM is paged into
         * two 1 MB buffers with an LRU that NULLs the entry it evicts
         * (gba_memory.c: evict_gamepak_page), and the mixer reads its PCM sample
         * data straight out of ROM. So it happens, and returning 0 here is not a
         * safe way to say so.
         *
         * The windows are all held at the door, before a single byte is written —
         * except that the sample window walks forward and re-maps when it leaves
         * its 32 KB page, which is mid-block, AFTER mix writes have landed. A
         * failure there returns M4A_DECLINED and cpu.cpp then interprets the block
         * from the top, mixing that span a second time. Quiet, wrong audio.
         *
         * So do exactly what the interpreter does at exactly the same point —
         * read_memory_gamepak: `if(!map) map = load_gamepak_page(index & 0x3FF)`.
         * That restores the invariant the transliteration was proven under.
         *
         * Safe against evicting a window out from under us: of the four windows
         * only the sample one is ever on the cart (the other three are checked
         * writable_region() first, so IWRAM/EWRAM), and map_rom_entry only ever
         * clears cart entries. The page this call evicts is therefore never one
         * another live window is holding. */
