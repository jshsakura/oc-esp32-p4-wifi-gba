...
> ⚠️ **2026-08-02: 이 절의 전제를 다시 봐야 한다.** 사용자 본인의 STM32H7 포트
> (`~/app/game-and-watch-retro-go-sd`)가 **같은 gpSP 코어로 GBA 60fps를 내고 있고, 다이나렉
> 없이 냈다.** 그쪽 `docs/OPTIMIZATION_LEDGER.md`의 GBA 항목:
>
> - **"M4A 믹서 HLE가 핵심 레버다 — 게스트 시간의 27~60%."** GBA 게임 대부분이 사운드
>   믹서를 소프트웨어로 돌리는데, 그걸 통째로 걷어내고 네이티브 재생기로 대체했다.
>   **아키텍처 무관한 코어 코드라 그대로 가져올 수 있다.**
> - **"다이나렉 — 닫힘. gpSP의 백엔드는 x86/ARM32/A64/MIPS뿐이고 Thumb-2 백엔드는 존재하지
>   않는다. JIT 캐시 예산 논쟁은 전부 무의미하다. 한 세션에서 두 번 반박됐다."**
>   우리 사정도 같다 — RISC-V 백엔드도 없고, 5절이 그걸 처음부터 쓰는 작업량 산정이다.
>
> 즉 **아래 계획(2c~2e, 다이나렉 복원 + RV32 백엔드 작성)보다 HLE가 먼저다.** 27~60%면
> 지금 13~16fps가 20~40fps로 가고, 거기에 아이들 루프 스킵(`Core/Src/porting/gba/gba_idle_loop.c`)이
> 더 붙는다. 다이나렉은 그 다음에 판단할 문제다.
>
> **같은 문서가 다른 시스템도 이미 답을 갖고 있다** — SNES는 APU(SPC700+DSP)를 HLE로 빼서
> **46 → 55.4fps**(우리 현재 26~27), 원더스완은 아이들 스킵으로 에뮬레이션의 **77%**를 걷어냈다.
> "순수 연산량이라 방법이 없다"는 판정은 그 문서를 읽기 전에는 내리지 말 것.
>
> **성능 작업을 시작하기 전에 `~/app/game-and-watch-retro-go-sd/docs/OPTIMIZATION_LEDGER.md`를
> 먼저 읽어라.** 이미 끝까지 가본 길과 그 이유가 시스템별로 적혀 있다.
