...
[BRINGUP.md](BRINGUP.md) A9·A11이며, **모두 디스플레이 블릿(13ms/프레임)을 포함한 값**이다.
...
- **CPU 블릿 제거.** `components/retro-go/drivers/display/st7701.h`의 `lcd_send_buffer()`가 회전·바이트스왑을 픽셀 단위 루프로 처리 중. Phase 2의 전제조건.
  ...
  - 후보 1: **PPA / 2D-DMA** (스케일·회전) — **2026-08-01 구현됨**. `rg_display.c`의
    ...
    - **2026-08-02 실측 A/B.** `rg_display_bench.c`가 기종 크기별 팔레트 서피스를 60프레임씩
      ...
      | 기종 | 소스 → 출력 | CPU | PPA | |
      |---|---|---|---|---|
      | PICO-8 | 128×128 → 480×480 | 20.67ms | **9.88ms** | 2.1배 |
      | GBA | 240×160 → 720×480 | 52.58ms | **23.51ms** | 2.2배 |
      | NES/SNES | 256×224 → 512×448 | 37.60ms | **27.15ms** | 1.4배 |
      | 메가드라이브 | 320×224 → 640×448 | 48.02ms | **34.37ms** | 1.4배 |
      | 게임앤워치 | 320×240 → 640×480 | 49.74ms | **37.03ms** | 1.3배 |

      **1.3~2.2배 빨라졌지만 60fps 예산 16.6ms에는 아직 아무도 못 들어온다.** 그리고 PPA 쪽
      효율이 출력 픽셀당 0.043µs(PICO-8)에서 0.118µs(NES)로 3배 벌어진다 — **회전이 DMA 쓰기를
      세로로 만들어 픽셀마다 캐시라인을 하나씩 쓰는 것**이 유력한 원인이고, CPU 전치가 비쌌던
      이유와 같다. 다음에 확인할 것: 회전 0으로 같은 크기를 재서 회전 비용만 분리할 것.
