...
2026-08-01에 22개 탭을 전부 실기에 태우고 나서 갱신했다. 실측치의 출처는
[BRINGUP.md](BRINGUP.md) A9·A11이다.

> ⚠️ **아래 fps는 전부 다시 재야 한다.** 두 가지가 그날 이후 바뀌었다 — 8/2에 **PSRAM이 20MHz로
> 묶여 있던 것을 찾아 80MHz로 올렸고**(A14), 디스플레이 블릿이 CPU에서 DMA로 넘어갔다. 롬도
> 코어 워킹셋도 PSRAM에 있으므로 **아래 숫자는 메모리가 4분의 1로 묶인 상태의 값이다.** 특히
> 티어 B로 내려간 것들(메가드라이브 15.6, SNES 27.2, NGPC 42)은 판정이 바뀔 수 있다.
> 버튼 없이 게임을 띄우는 수단이 준비되는 대로 다시 잰다.

| 등급 | 기종 | 판정 |
|---|---|---|
| **A. 여유** | NES, GB/GBC, PC엔진, 게임앤워치, 아타리 2600 | 실측 55~100fps, BUSY 12~58% |
| **A−. 무난** | SMS/GG/SG-1000/ColecoVision, MSX, PICO-8 | 실측 49~56fps. 30fps 카트는 여유, 60fps 카트는 걸친다 |
| **B. 여유 없음** | 네오지오 포켓 컬러 | 42fps에 CPU 100%. 패널이 붙으면 더 떨어진다 |
| **B. 실질 최대치** | **GBA**, 메가드라이브, SNES | 실측 16.4 / 15.6 / 27.2fps. `gwenesis`도 무난하지 않았다. **GBA가 본진**이며 다이나렉이 관건 |
| **미측정** | 포켓몬 미니, 원더스완, 버추얼 보이, Lynx, DOOM | 이식은 끝났고 롬이 없어 아직 못 쟀다. 슈퍼비전은 32.4fps인데 BUSY 22% — 코어 쪽 프레임 페이싱으로 보이며 확인 필요 |
...
| 대상 | 20MHz PSRAM (07-31) | 80MHz PSRAM (08-02) |
|---|---|---|
| PSRAM, 캐시 상주 | 346 MIPS | **344 MIPS** — 캐시라 안 변한다 |
| PSRAM, 스트리밍(1MB를 블록마다 한 번씩) | 8 MIPS (43배 차) | **21 MIPS (16배 차)** |
| 일반 힙의 내부 RAM (`MALLOC_CAP_INTERNAL`) | Instruction access fault | 같음 |

**07-31 측정은 PSRAM이 20MHz로 묶인 상태였다**(A14). 80MHz에서 다시 재니 스트리밍이 2.6배
빨라졌고, **설계를 가르는 비율이 43배에서 16배로 내려왔다.**
...
3. **설계를 가르는 숫자는 16배다.** 코드캐시를 PSRAM에 두는 것 자체는 가능하지만, L2 128KB 안에
   머무는 블록만 전속력이다. 즉 캐시 크기가 곧 성능이고, 코드캐시는 L2에 맞춰 설계하고 PSRAM은
   넘칠 때의 저장소로 보는 것이 맞다. **다만 넘쳤을 때의 벌금이 절반 이하로 줄었다** — 43배일 때는
   L2 밖으로 나가는 것이 곧 파탄이었지만 16배면 감당 가능한 꼬리다. Phase 2 코드캐시 설계를
   시작할 때 이 숫자로 다시 계산할 것.
...
- **CPU 블릿 제거.** `components/retro-go/drivers/display/st7701.h`의 `lcd_send_buffer()`가 회전·바이트스왑을 픽셀 단위 루프로 처리 중. Phase 2의 전제조건.
  ...
  - ⚠️ **위 표는 회전을 빼고 잰 값이다.** 패널이 없으면 `st7701_ctx.framebuffer`가 NULL이라
    `lcd_send_buffer()`가 첫 줄에서 돌아가고, **논리 행을 패널 열로 바꾸는 전치 — 블릿의 비싼
    절반 — 이 아예 안 돈다.** 패널 없이도 목적지를 갖도록 스크래치 프레임버퍼를 잡고 다시 재니
    같은 GBA 프레임이 7.6ms가 아니라 **52.6ms**였다. 아래 8/2 표가 진짜 값이다.
  - 후보 1: **PPA / 2D-DMA** (스케일·회전) — **2026-08-01 구현됨**. `rg_display.c`의
    `ppa_write_update()`가 스케일·270도 회전·저장을 트랜잭션 하나로 처리하고 패널
    프레임버퍼에 직접 쓴다. CPU는 픽셀을 건드리지 않는다.
    - 하드웨어가 **8비트 팔레트를 못 받는다**(SRM 컬러모드는 ARGB8888/RGB888/RGB565/YUV420뿐,
      실리콘에 CLUT 모드가 없다. 블렌딩 엔진에는 레지스터가 있으나 그쪽은 스케일을 못 한다).
      그래서 팔레트만 CPU가 편다 — **출력이 아니라 소스 한 번**이라 최대 9분의 1이다.
    - 배율은 1/16 단위(정수부 8비트+소수부 4비트)라 3.75배도 정확히 들어간다. 나누어떨어지지
      않는 배율과 필터·스캔라인은 CPU 경로로 폴백한다.
    - **2026-08-02 실측 A/B.** `rg_display_bench.c`가 기종 크기별 팔레트 서피스를 60프레임씩
      밀어넣고 디스플레이 태스크의 busy 카운터를 나눈다. 롬도 패널도 버튼도 필요 없다.
      ...
      **PPA 열만 60fps 예산 16.6ms 안에 전부 들어온다.** CPU 열은 GBA·메가드라이브·게임앤워치가
      모두 바깥이다. 즉 DMA 블릿과 A14의 PSRAM 클럭은 둘 중 하나가 아니라 둘 다 필요했다.

      가는 길에 **가설 두 개가 계측에 뒤집혔다**(BRINGUP A13). 회전이 비쌀 것이라고 했는데 끄고
      재보니 4% 차이였고, 팔레트 확장이 비쌀 것이라고 했는데 DMA 쪽이 15배였다. 진짜 원인은
      셋째 것 — **PSRAM이 20MHz로 돌고 있었다**(A14).
    - **값은 SRAM으로 치렀다.** `esp_driver_ppa`를 링크하니 retro-core가 775바이트 넘쳤다.
      `sram_low`는 183KB뿐인데 IRAM 텍스트가 이미 176KB다(에뮬레이터 코어들이 핫루프를 거기
      올린다 — snes9x 혼자 41KB). 힙 코드 7.9KB를 플래시로 내려 갚았다
      (`CONFIG_HEAP_PLACE_FUNCTION_INTO_FLASH`). 스케일링 LUT 4.4KB도 PSRAM으로 갔다.
...
- [x] **Q1: P4에서 PSRAM 영역 명령어 인출 가부** — 2026-07-31 답 나옴. 된다(캐시 상주 344 MIPS,
      스트리밍 **21 MIPS**, 2026-08-02에 80MHz PSRAM으로 재측정). 다만 **내부 RAM 쪽이 폴트난다.**
      3절 참조
