...
### A15. 20MHz로 되돌렸다, 그리고 코어별 수치를 다시 쟀다 — 2026-08-02

**80MHz는 못 쓴다.** A14의 3배는 실측이 맞지만 **retro-core가 부트루프에 빠진다** — 새로 쓴
파티션에서도, 플래시를 120MHz로 두든 40MHz로 두든 똑같이 재현된다. 2단계 부트로더가
`entry 0x4ff29ed0` 다음 자기 로그를 한 줄도 못 찍고 워치독에 잡힌다. **앱 코드는 시작조차 못
한다** — 1.7MB 이미지를 읽는 로더 단계다. 나머지 앱은 전부 1MB 근처고 80MHz에서 멀쩡히 돈다.
retro-core가 22개 탭 중 11개를 갖고 있으므로 이대로는 못 쓴다.

**다음에 볼 것:** esp-idf의 MSPI 타이밍 진단은 `ESP_EARLY_LOGD`라 이 프로젝트 로그 레벨에서
컴파일 아웃된다. 로그 레벨을 한 번만 올려 빌드해서, **런처 부팅과 retro-core 부팅이 고르는
튜닝 포인트를 비교**할 것.

#### 코어별 fps — 호스트에서 롬을 지정해 부팅시켜 잰 값 (0.2-1)

전부 PSRAM 20MHz, PPA 블릿, 스크래치 프레임버퍼 있음(= 전치까지 도는 진짜 디스플레이 경로).

| 코어 | 앱 | FPS | BUSY |
|---|---|---|---|
| PC엔진 | retro-core | 58~60 | 38~59% |
| NES | retro-core | 55~59 | 11~14% |
| 네오지오 포켓 컬러 | ngpocket-go | 32~33 | 96% |
| SNES | retro-core | 26~27 | 96% |
| GBA (인터프리터) | gbsp | 13~16 | 100% |
| **메가드라이브** | gwenesis | **3~4** | **100%** |

**A9의 수치와 직접 비교하지 말 것.** 그때는 프레임버퍼가 NULL이라 전치가 안 돌았다(A13).

#### 메가드라이브 3~4fps — 회귀가 아니다

A9에 15.6fps로 적혀 있지만 재현되지 않는다. 확인한 것:

- 롬은 262,656바이트로 **A9이 쓴 256KB와 같은 크기**다
- 디스플레이 탓이 아니다 — 프레임버퍼를 다시 NULL로 만들어도 3~4fps다
- PSRAM 속도 탓이 아니다 — 20MHz와 80MHz가 같다
- **이번 주 변경 탓이 아니다** — 커밋 `494633f`(디스플레이·메모리 작업 이전)로 gwenesis를 빌드해
  실기에 태워도 3~4fps다

즉 **원래 이랬고 A9의 15.6은 설명되지 않는다.** 그리고 이건 하드웨어 한계가 아니다 —
같은 코어가 `~/app/game-and-watch-retro-go-sd`의 **STM32H7(280MHz 단일 M7, PSRAM 없음)에서
정상 속도로 돈다.** 더 약한 하드웨어에 4~5배 지는 것은 이식 쪽 구조 문제다. 두 포트를 나란히
놓고 비교하는 중.
