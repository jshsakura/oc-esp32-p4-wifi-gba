...
| PC엔진 (pce-go) | 128KB·256KB 둘 다 | — | — | **조용히 재부팅** |
...
- **PC엔진**: retro-core가 뜨고 `rg_system_init` 안에서 "Retro-Go ready"를 찍기 전에 **아무 메시지
  없이 재부팅**한다. 패닉 텍스트도 없다. 롬 두 개(128KB/256KB)에서 동일하므로 롬이 아니라 코어
  쪽이다. `configNs`가 `pce`일 때만 발생.
