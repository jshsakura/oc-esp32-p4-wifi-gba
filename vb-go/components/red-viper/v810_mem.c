...
#ifdef GNW_VB_DEVICE
// Device: ROM is XIP'd from flash (no 16MB mirror buffer). A power-of-2 ROM
...
uint64_t mem_vip_rbyte(WORD addr) {
    ...
    addr &= 0x7ffff;
    if(!(addr & 0x40000)) {
        if (addr < BGMAP_OFFSET && (addr & 0x6000) != 0x6000 && !vb_state->tVIPREG.drawing) video_download_vip(!vb_state->tVIPREG.tDisplayedFB);
        wait = 4LL << 32;
        ...
        if (likely(emulating_self)) {
            if(addr < BGMAP_OFFSET) {
                ...
                if((addr & 0x6000) == 0x6000) {
                    ...
                    if (!vb_state->tVIPREG.drawing) video_download_vip(!vb_state->tVIPREG.tDisplayedFB);
                ...
                tDSPCACHE.ObjDataCacheInvalid=1;
                #endif
            } else if((addr >=BGMAP_OFFSET)&&(addr < (BGMAP_OFFSET+(16*BGMAP_SIZE)))) { //Writes to BGMap Table
                #ifdef NEED_BG_CACHE
                tDSPCACHE.BGCacheInvalid |= 1 << ((addr-BGMAP_OFFSET)/BGMAP_SIZE);
                #endif
            }
        }
...
uint64_t mem_rom_rbyte(WORD addr) {
    ...
    return (WORD)((SBYTE *)(V810_ROM1.off + (addr & (0x07000000 | VB_ROM_MASK))))[0] | wait;
}

uint64_t mem_rom_rhword(WORD addr) {
    uint64_t wait = (uint64_t)(2 - (vb_state->tHReg.WCR & 1)) << 32;
    return (WORD)((SHWORD *)(V810_ROM1.off + (addr & (0x07000000 | VB_ROM_MASK) & ~1)))[0] | wait;
}

uint64_t mem_rom_rword(WORD addr) {
    uint64_t wait = (uint64_t)(2LL - (vb_state->tHReg.WCR & 1)) << 33;
    return ((WORD *)(V810_ROM1.off + (addr & (0x07000000 | VB_ROM_MASK) & ~3)))[0] | wait;
}
